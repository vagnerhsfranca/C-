...
/*---------------QuickSort-------------------*/ //ORDENA DO MENOR PARA O MAIOR
...
/*------------------------Revert----------------------*/ //ORDENA DO MAIOR PARA O MENOR
int partitionRevert(int * array, int start, int finish) {
	int pivo = array[finish];//pivo da particao
	int p = start - 1;
	int temp;
...
void revert(int * array, int start, int finish) {
	if (finish <= start) return;
	int pivot = partitionRevert(array, start, finish);
	revert(array, start, pivot - 1);
	revert(array, pivot + 1, finish);
}

void revert(int * array, int size) {
	revert(array, 0, size - 1);
}
/*----------------------------------------------------*/
